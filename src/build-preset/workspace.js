...
export function getWorkspacePackages(options = {}) {
    const { startDir = process.cwd(), workspaceFilePath, onlyExternal = false } = options;
    const wsFile = workspaceFilePath ?? findWorkspaceFile(startDir);
    if (!wsFile || !existsSync(wsFile))
        return [];
...
export function createWorkspaceAlias(packages) {
    return Object.fromEntries(packages.map(pkg => [pkg.name, pkg.srcDir]));
}
export function createFsAllow(root, packages) {
    return [root, ...packages.map(pkg => pkg.dir)];
}
export function createTailwindContent(baseContent, packages) {
    return [
        ...baseContent,
